...
Signature::Signature(TreeNode bg){
	treeBag = bg;
...
// GETTERS
int Signature::getIndex(unsigned u){
...
unsigned Signature::getSticker(unsigned u){
...
vector<unsigned> Signature::getBag(){
	return TreeBag;
}
...
vector<unsigned> Signature::getSelected(){
	return selected;
}
	

// SETTERS
void Signature::setSticker(unsigned u, unsigned stk){
	int index = getIndex(u);
...
void Signature::select(unsigned u){
